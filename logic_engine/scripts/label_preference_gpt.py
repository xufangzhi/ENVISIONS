...
def main():
    ...
    pred = []
    with open(f"../open-instruct/data/proofwriter_gpt-4-0125-preview.jsonl",'r') as file:
        for line in file:
            pred.append(json.loads(line))
    print(len(pred), len(gold))
    assert len(pred)==len(gold), "length mismatch"
    ...
    for i in range(len(gold)):
        ...
        if answer==ground_truth:
            data_dict = {}
            num += 1
            data_dict['prompt'] = pred[i]['prompt']
            data_dict['completion'] = logic_program
            train_data_new.append(data_dict)
    print(len(train_data_new))

    with jsonlines.open(f"../open-instruct/data/proofwriter_gpt-4-0125-preview.jsonl",'w') as file:
        for i in range(len(train_data_new)):
            file.write(train_data_new[i])
