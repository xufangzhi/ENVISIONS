    ...
    for iter_idx in range(args.cur_iter+1):
        ...
        for i in range(len(scores)):  # for each origin sample, origin id: i
            ...
            for j in range(5):  # generate 5 code for each (x,y)
                ...
                if scores[i][j] == 1 and response not in response_pool and ground_truth[i]['input'].strip() in response:
            ...
            if chosen_candidates_set:
                ...
                # [SFT] normal sft data
                for m in range(min(2, max(1,len(chosen_candidates_list)-1))):
                    data_dict = {}
                ...
                if 1<len(chosen_candidates_list)<5 and len(rejected_candidates_list)>0:
                    ...
                    data_dict['completion'] = extract_code_blocks(candidates[chosen_candidates_list[-1]]['response'])
