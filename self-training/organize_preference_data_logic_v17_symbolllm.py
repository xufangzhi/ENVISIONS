    ...
    for iter_idx in range(args.cur_iter+1):
        ...
        for i in range(len(scores)):  # for each origin sample, origin id: i
            ...
            for j in range(5):  # generate 5 code for each (x,y)
                response = extract_ans(candidates[i*5+j]['response'])
            ...
            if chosen_pool[str(i)]:
                ...
                for m in range(min(10, max(1,len(chosen_pool[str(i)])-1))):  # choose at most 10 candidates
                    ...
                    data_dict['prompt'] = "Generate the logical representation for the given context and question.\n" + "\nThe context is:" + \
                                          ground_truth[i]['context'] + "\nThe question is:\n" + ground_truth[i]['question'] + "\nThe logical representation is\n"
                    data_dict['completion'] = extract_ans(chosen_pool[str(i)][m]['response'])
                    explore_num += 1
                ...
                for m in range(repair_num):
                    ...
                    data_dict['prompt'] = "You are provided with a logical representation to solve the given problem. You can either repair and refine it, or you can simply return the original one.\n" + \
                                          "\nThe context is:" + ground_truth[i]['context'] + "\nThe question is:" + ground_truth[i]['question'] + "\nThe current logical representation is:\n" + \
                                          extract_ans(rejected_pool[str(i)][m]['response']) + "\nThe logical representation is:\n"
                    data_dict['completion'] = extract_ans(chosen_pool[str(i)][explore_num+m]['response'])
