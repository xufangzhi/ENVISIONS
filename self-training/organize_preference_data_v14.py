...
def main():
    ...
    for iter_idx in range(args.cur_iter+1):
        ...
        for i in range(len(scores)):  # for each origin sample, origin id: i
            ...
            if chosen_candidates_set:
                ...
                # [SFT] normal sft data
                for m in range(min(1, max(1,len(chosen_candidates_list)-1))):  # choose the first
                    data_dict = {}
                ...
                if 1<len(chosen_candidates_list)<5 and len(rejected_candidates_list)>0:
                    ...
                    for m in range(1, len(chosen_candidates_list)): # ignore the first one for normal sft
                        for n in range(len(rejected_candidates_list)):
                            try:
                                codeblue_score_cur = calculate_code_bleu(preference_data[chosen_candidates_list[m]]['chosen'],
                                                                     preference_data[i]['rejected'])
                            except:
                                codeblue_score_cur = 0
                            if codeblue_score_cur>=codeblue_score:
                                selected_pair = (chosen_candidates_list[m],rejected_candidates_set[n])
                                codeblue_score = codeblue_score_cur
                    if selected_pair != (None,None):
                        ...
                        data_dict['prompt'] = "Repair the provided Python code to solve the given problem.\n" + "\nThe question is:" + \
                                              ...
                                              extract_code_blocks(candidates[selected_pair[1]]['response']) + \
                                              "\nThe repaired code is as follows:\n"
                        data_dict['completion'] = extract_code_blocks(candidates[selected_pair[0]]['response'])
    ...
    with open(f"symbol-llm-v2/logs/{args.task_prefix}_log.txt","a") as file:
        # file.write(f"orginal effective samples: {count_effective_samples(scores)}")
