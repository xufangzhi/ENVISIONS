...
def parse_args():
    ...
    parser.add_argument(
        "--task_prefix",
        type=str,
        default="gsm_math_full_star_llama2chat13b",
...
def main():
    ...
    for cur_iter in range(2,args.iter_num):
        logger.info(f"Current iteration: {cur_iter}")
        # ======================================================================== #
        #                     Step 1: Generate Samples
        # ======================================================================== #
        logger.info(f"Start to generate samples for iteration-{cur_iter}")
        subprocess.call(["python", f"/cpfs01/user/xufangzhi/symbol-llm-v2/self-training/organize_preference_data_star_{base_model}.py", \
        ...
        for part_id in range(1,9):
            env = os.environ.copy()
            env['CUDA_VISIBLE_DEVICES'] = str(part_id - 1)
            process = subprocess.Popen(['python', 'symbol-llm-v2/generate_symbol_output/generate_candidates_vLLM_self_training.py', \
        ...
        for part_id in range(1,9):
            env = os.environ.copy()
            env['CUDA_VISIBLE_DEVICES'] = str(part_id - 1)
            process = subprocess.Popen(['python', 'symbol-llm-v2/generate_symbol_output/generate_repaired_math_vLLM_self_training.py', \
